...
const DashboardNavigation: React.FC<DashboardNavigationProps> = ({ activeTab, setActiveTab }) => {
  const { translated: overviewLabel } = useTranslation("Overview");
  const { translated: coursesLabel } = useTranslation("Courses");
  const { translated: myCoursesLabel } = useTranslation("My Courses");
  const { translated: assignmentsLabel } = useTranslation("Assignments");
  const { translated: materialsLabel } = useTranslation("Materials");
  const { translated: discussionsLabel } = useTranslation("Discussions");
  const { translated: analyticsLabel } = useTranslation("Analytics");
    ...
    <div className="overflow-hidden mb-6">
      <ScrollArea className="w-full" orientation="horizontal">
        <TabsList className="flex bg-edu-card-bg min-w-max">
          ...
          <TabsTrigger value="my-courses" className="flex flex-col items-center gap-1 py-3 min-w-[100px]">
            <GraduationCap size={18} />
            <span className="text-xs">{myCoursesLabel}</span>
          </TabsTrigger>
